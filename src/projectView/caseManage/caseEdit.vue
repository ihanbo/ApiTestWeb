        ...
        <el-tab-pane label="用例信息" name="first" style="margin-top: 10px">
          <el-form size="small" :inline="true">
            <el-form-item :required="true" label="用例名称" :label-width="caseData.formLabelWidth">
              <el-input v-model="caseData.name" style="width: 150px"></el-input>
            </el-form-item>
            <el-form-item label="项目选择" :label-width="caseData.formLabelWidth">
              <el-select
                v-model="form.pjname"
                placeholder="请选择项目"
                @change="changeSetChoice"
                style="width: 150px"
              >
            ...
            <el-form-item label="用例编号" :label-width="caseData.formLabelWidth" v-if="caseData.id">
              <el-input v-model.number="caseData.num" :minlength="215" style="width: 130px"></el-input>
            </el-form-item>
          </el-form>
          <el-form :inline="true" size="small">
            <el-form-item label="用例描述" :label-width="caseData.formLabelWidth">
              <el-input v-model="caseData.desc" style="width: 150px"></el-input>
            </el-form-item>
            ...
            <el-form-item label="集合选择" :label-width="caseData.formLabelWidth">
              ...
              <!--                                       style="width: 150px">-->
              <el-select
                v-model="form.caseSetName"
                placeholder="请选择用例集"
                value-key="id"
                @change="changeCaseSetChoice"
                style="width: 150px"
              >
                <el-option
            ...
            <el-form-item label="执行次数" label-width="70px">
              <el-input-number v-model="caseData.times" :min="1" :max="1000"></el-input-number>
            </el-form-item>
            <el-form-item label="等待时间" label-width="70px">
              <el-input-number v-model="caseData.waitTimes" :min="0" :max="10000"></el-input-number>
            </el-form-item>
          </el-form>
          <hr
          ...
          <el-form :inline="true" class="demo-form-inline" size="small">
            <el-form-item label=" " labelWidth="10px">
              <el-select
                v-model="form.sceneVariableProjectName"
                placeholder="请选择项目"
            ...
            <el-form-item>
              <el-button type="primary" size="small" @click.native="addConfigData()">添加配置</el-button>
          ...
          >
            <el-table-column property="key" label="Key" header-align="center" minWidth="100">
              <template slot-scope="scope">
                <el-input v-model="scope.row.key" size="small" placeholder="key"></el-input>
              </template>
            </el-table-column>
            ...
            <el-table-column property="value" label="操作" header-align="center" width="140">
              <template slot-scope="scope">
                <el-button-group>
                  <el-button
                    type="info"
                    icon="my-icon-jiantou-xiangshang"
                    @click.native="upNum(scope.$index)"
                    size="mini"
                  >{{null}}</el-button>
                  <el-button
                    type="info"
                    icon="my-icon-jiantou-xiangxia"
                    @click.native="downNum(scope.$index)"
                    size="mini"
                  >{{null}}</el-button>
                  <el-button
                    type="danger"
                    size="mini"
                    @click.native="delConfigVariable(scope.$index)"
                  >删除</el-button>
                </el-button-group>
              </template>
            </el-table-column>
        ...
        <el-tab-pane label="执行步骤" name="second">
          <el-row :gutter="24">
            <el-col :span="stepSpan">
              ...
              <draggable
                v-model="caseData.apiCases"
                :options="{group:'apiData',animation:300}"
                style="width: 99%;min-height: 10px;"
              >
                <div
                  v-for="(_data, index) in caseData.apiCases"
                  :key="index"
                  class="list-complete-item"
                >
                  <el-row :gutter="21">
                    <el-col :span="2" style="padding-top: 3px">
                      <el-switch v-model="caseData.apiCases[index]['status']"></el-switch>
                    </el-col>
                    <el-col :span="5">
                      <el-input v-model="caseData.apiCases[index]['case_name']" size="mini"></el-input>
                    </el-col>
                    <el-col
                      :span="4"
                    ...
                    <el-col :span="8" style="padding-left: 50px;padding-top: 3px">
                      <el-button type="danger" size="mini" @click.native="delApiCase(index)">删除</el-button>
                      <el-button
                        type="primary"
                        size="mini"
...
<!--              <el-row
              ...
              <draggable v-model="ApiMsgData" :options="this.draggableOptions">
                <transition-group name="list-complete">
                  <div
                    v-for="(_data, index) in ApiMsgData"
                    :key="_data.num"
                    class="list-complete-item"
                  >
                    <el-row :gutter="24">
                      ...
                      >{{ _data.desc }}</el-col>
                      <el-col :span="8">{{ _data.url }}</el-col>
                    </el-row>
                  </div>
                </transition-group>
              </draggable>
    ...
    <apiMsgDataEdit :apiCases="caseData.apiCases" ref="apiMessageEditFunc"></apiMsgDataEdit>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    synchronousData() {
      let index = this.allSetList[this.projectName]
        .map(item => item.id)
        .indexOf(this.setTempData.setId);
      ...
      if ("id" in this.caseData.apiCases[i]) {
        this.$confirm(
          "是否删除用例中已保存的步骤：" +
            ...
            this.caseData.apiCases[i]["case_name"] +
            "</strong>" +
            "?",
          "提示",
          {
    ...
    addConfigData() {
      //  复制配置信息到用例配置里面
      this.$axios
        .post(this.$api.configDataApi, {
          configId: this.form.config.configId
        })
        .then(response => {
          this.caseData.variable = this.caseData.variable.concat(
            response.data["data"]["variables"]
          );
          this.caseData.variable = JSON.parse(
            JSON.stringify(this.caseData.variable)
          );
          this.caseData.funcAddress = response.data["data"]["func_address"];
        });
    },
